...
.PHONY: lint-web
lint-web: venv
	$(VENV_BIN)/$(PYTHON) -m pylint --rcfile=pylintrc reforis_remote_access
	$(VENV_BIN)/$(PYTHON) -m pycodestyle --config=pycodestyle reforis_remote_access
...
create-messages: venv
	$(VENV_BIN)/pybabel extract -F babel.cfg -o ./reforis_remote_access/translations/messages.pot . --project=$(PROJECT) --version=$(VERSION) --copyright-holder=$(COPYRIGHT_HOLDER) --msgid-bugs-address=$(MSGID_BUGS_ADDRESS)

.PHONY: update-messages
update-messages: venv
	$(VENV_BIN)/pybabel update -i ./reforis_remote_access/translations/messages.pot -d ./reforis_remote_access/translations --update-header-comment
...
clean:
	...
	rm -rf $(JS_DIR)/node_modules/ reforis_static/reforis_remote_access/js/app.min.js
	$(PYTHON) -m pip uninstall -y reforis_remote_access
